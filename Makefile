.PHONY: build test test-unit test-integration lint docker-up docker-down \
        docker-test-up docker-test-down migrate swagger run run-pg dev

# Subir tudo: banco, migrações e API (comando principal)
dev:
...
# Subir banco de teste (porta 5433)
docker-test-up:
...
# Rodar API em modo desenvolvimento (in-memory)
run:
	go run ./cmd/api/
