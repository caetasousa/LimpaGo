...
func (r *RepositorioPerfilPG) BuscarPerfilProfissional(ctx context.Context, usuarioID int) (*entity.PerfilProfissional, error) {
	q := `SELECT usuario_id, descricao, anos_experiencia,
	      ...
	      FROM perfis_profissional WHERE usuario_id = $1`
	...
	if err != nil {
		return nil, mapearErroPG(err, errosdominio.ErrPerfilProfissionalNaoEncontrado)
	}
...
func (r *RepositorioPerfilPG) SalvarPerfilProfissional(ctx context.Context, perfil *entity.PerfilProfissional) error {
	q := `INSERT INTO perfis_profissional
...
func (r *RepositorioPerfilPG) AtualizarPerfilProfissional(ctx context.Context, perfil *entity.PerfilProfissional) error {
	q := `UPDATE perfis_profissional
...
func (r *RepositorioPerfilPG) BuscarPerfilCliente(ctx context.Context, usuarioID int) (*entity.PerfilCliente, error) {
	q := `SELECT usuario_id,
	             ...
	             observacoes, profissional_preferido_id, criado_em, atualizado_em
	...
	err := obterExecutor(ctx, r.db).QueryRowContext(ctx, q, usuarioID).Scan(
		...
		&p.Observacoes, &p.ProfissionalPreferidoID, &p.CriadoEm, &p.AtualizadoEm,
	)
	if err != nil {
...
func (r *RepositorioPerfilPG) SalvarPerfilCliente(ctx context.Context, perfil *entity.PerfilCliente) error {
	q := `INSERT INTO perfis_cliente
	      (usuario_id, endereco_rua, endereco_complemento, endereco_bairro,
	       endereco_cidade, endereco_estado, endereco_cep,
	       tipo_imovel, quartos, banheiros, tamanho_imovel_m2,
	       observacoes, profissional_preferido_id)
	...
	return obterExecutor(ctx, r.db).QueryRowContext(ctx, q,
		...
		perfil.Observacoes, perfil.ProfissionalPreferidoID).
		Scan(&perfil.CriadoEm, &perfil.AtualizadoEm)
}
...
func (r *RepositorioPerfilPG) AtualizarPerfilCliente(ctx context.Context, perfil *entity.PerfilCliente) error {
	q := `UPDATE perfis_cliente
	      SET endereco_rua=$1, endereco_complemento=$2, endereco_bairro=$3,
	          ...
	          observacoes=$11, profissional_preferido_id=$12, atualizado_em=NOW()
	      WHERE usuario_id=$13
	      RETURNING atualizado_em`
